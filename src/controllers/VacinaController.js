...
async function getVacina(req, res) {
  try {
    if (!req.params.vacinaId) {
      return res
        .status(400)
        .send({ status: true, erros: ["Id da vacina nao informado."] });
    }

    const vacinas = await ModelVacina.VacinaPet.findById(
      req.params.vacinaId
    ).populate("classe_pets");
...
async function getPetVacinas(req, res) {
  try {
    if (!req.params.petId) {
      return res
        .status(400)
        .send({ status: true, erros: ["Id do pet nao informado."] });
    }

    const pet = await ModelPet.Pet.findById(req.params.petId).populate(
      "vacinas"
    );
    ...
    if (!pet) {
      return res
        .status(404)
        .send({ status: true, erros: ["Pet solicitado náo encontrado"] });
    }
...
async function getClasseVacinas(req, res) {
  try {
    if (!req.params.classeId) {
      return res
        .status(400)
        .send({ status: true, erros: ["Id da classe nao informado."] });
    }

    const vacinas = await ModelVacina.VacinaPet.find({
      classe_pets: req.params.classeId,
    });
  ...
  try {
    ...
    if (!nome || !dose || !classe_pets) {
      return res
        ...
        .send({
          status: false,
          erros: [`Atributos obrigatorios: nome, dose, classe_pets.`],
        });
    }
    ...
    if (await ModelVacina.VacinaPet.findOne({ nome })) {
      return res
        .status(400)
        .send({ status: false, erros: [`Vacina ${nome} ja existe.`] });
    }
...
async function updateVacina(req, res) {
  try {
    ...
    if (!vacina) {
      return res
        .status(404)
        .send({ status: false, erros: [`Vacina ${nome} náo encontrada.`] });
    }

    const vacinaUpdated = await ModelVacina.VacinaPet.findByIdAndUpdate(
      vacinaId,
      req.body,
      { new: true }
    );
    ...
    if (!vacinaUpdated) {
      return res
        .status(500)
        .send({ status: false, erros: ["Náo foi possivel atualizar."] });
    }
