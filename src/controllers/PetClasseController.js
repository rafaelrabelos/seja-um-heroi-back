...
async function insertPetClasse(req, res) {
  try {
    ...
    if (!nome) {
      return res
        .status(400)
        .send({ status: false, erros: [`Atributos obrigatorios: nome.`] });
    }

    if (await ModelClasse.ClassePet.findOne({ nome })) {
      return res
        .status(400)
        .send({ status: false, erros: [`Classe ${nome} ja existe.`] });
    }
  ...
  try {
    if (!nome || !descricao) {
      return res
        ...
        .send({
          status: false,
          erros: ["Dados obrigatorios: nome, decricao."],
        });
    }
    ...
    if (!petRaca) {
      return res
        .status(400)
        .send({ status: false, erros: ["Classe nao localizado."] });
    }

    const petClasseUpdated = await ModelClasse.ClassePet.findByIdAndUpdate(
      petClasseId,
      req.body,
      { new: true }
    ).populate("criadoPor");
