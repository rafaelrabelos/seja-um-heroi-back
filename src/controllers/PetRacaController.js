...
async function insertPetRacas(req, res) {
  ...
  try {
    if (!classe) {
      return res
        .status(400)
        .send({ status: false, erros: [`Atributos obrigatorios: classe.`] });
    }

    if (await ModelRacas.RacaPet.findOne({ nome })) {
      return res
        .status(400)
        .send({ status: false, erros: [`Raca ${nome} ja existe.`] });
    }
  ...
  try {
    if (!nome || !descricao || !classe) {
      return res
        ...
        .send({
          status: false,
          erros: ["Dados obrigatorios: nome, decricao, classe"],
        });
    }
    ...
    if (!petRaca) {
      return res
        .status(400)
        .send({ status: false, erros: ["Raca nao localizado."] });
    }

    const petRacaUpdated = await ModelRacas.RacaPet.findByIdAndUpdate(
      petRacaId,
      req.body,
      { new: true }
    ).populate("criadoPor");
